...
const BOT_TOKEN = '8355946636:AAHuzTniHg2FisQyz2DBCIfk0MlaEcF38FY';
const ADMIN_ID = 1421302016;
...
async function reorderServers(userId) {
    ...
    // رتب حسب createdAt إذا موجود، وإذا لا استعمل ترتيب عادي
    servers.sort((a, b) => {
...
async function startBedrockBot(ctx, server, versions) {
    ...
    try {
        ...
        if (ctx) {
            try {
                await ctx.editMessageText(`⏳ جاري الاتصال بالسيرفر مباشرة...`);
            } catch (e) { /* ignore */ }
        }

        // الحصول على إصدار افتراضي بدلاً من التحقق من السيرفر
        let mcVersion = '1.20.80'; // إصدار افتراضي
        const defaultVersion = 671; // بروتوكول 1.20.80
        
        // أو استخدم أول إصدار في القائمة
        const versionList = await getSupportedVersions();
        if (Object.keys(versionList.bedrock).length > 0) {
            const firstProtocol = Object.keys(versionList.bedrock)[0];
            mcVersion = versionList.bedrock[firstProtocol];
        }
        
        console.log(`🚀 محاولة الاتصال بـ ${server.ip}:${server.port} بالإصدار ${mcVersion}`);

        // استخدام الدالة من الملف المنفصل أو الكود المباشر
        connectToServer(server.ip, server.port, server.botName, mcVersion);
        
        // تحديث الحالة فوراً
        await Servers.updateOne({ _id: server._id }, { $set: { status: 'جاري المحاولة' } });

    } catch (error) {
        console.error(`❌ فشل البدء: ${error.message}`);
        await Servers.updateOne({ _id: server._id }, { $set: { status: 'فشل البدء' } });
    }
}
...
async function startBedrockBot(ctx, server, versions) {
    ...
    try {
        ...
        console.log(`🚀 محاولة الاتصال بـ ${server.ip}:${server.port}`);
        
        // استخدم الكود المباشر هنا
        connectDirectly(ctx, server);
...
// أضف هذه الدالة الجديدة في نفس الملف
async function connectDirectly(ctx, server) {
    try {
        const bedrock = require('bedrock-protocol');
        ...
        const SUPPORTED_VERSIONS = {
            "1.21.90": 818, "1.21.80": 800, "1.21.70": 786, "1.21.60": 776,
            "1.21.50": 766, "1.21.42": 748, "1.21.30": 729, "1.21.20": 712,
            "1.21.2": 686, "1.21.0": 685, "1.20.80": 671, "1.20.71": 662,
            "1.20.61": 649, "1.20.50": 630, "1.20.40": 622, "1.20.30": 618,
            "1.20.10": 594, "1.20.0": 589, "1.19.80": 582, "1.19.70": 575,
            "1.19.63": 568, "1.19.50": 560, "1.19.30": 554, "1.19.20": 544,
            "1.19.1": 527, "1.18.30": 503, "1.18.0": 475, "1.17.10": 448,
            "1.16.201": 422
        };

        // جرب إصدارات مختلفة
        const versionsToTry = ['1.20.80', '1.19.80', '1.18.30', '1.16.201'];
        
        for (const version of versionsToTry) {
            try {
                console.log(`🔄 جرب الإصدار ${version}...`);
                
                const client = bedrock.createClient({
                    host: server.ip,
                    port: server.port,
                    username: server.botName || 'MinecraftBot',
                    version: version,
                    offline: true,
                    connectTimeout: 8000,
                    skipPing: true  // تخطي ping للسرعة
                });

                client.once('spawn', async () => {
                    console.log(`✅ نجح الاتصال! الإصدار المناسب: ${version}`);
                    await Servers.updateOne({ _id: server._id }, { $set: { status: 'نشط' } });
                    
                    if (ctx) {
                        try {
                            await ctx.editMessageText(`🎉 البوت متصل الآن!`);
                ...
                client.once('error', (error) => {
                    console.log(`❌ فشل مع الإصدار ${version}: ${error.message}`);
                    client.disconnect();
                });

                // انتظر 5 ثواني قبل تجربة الإصدار التالي
                await new Promise(resolve => setTimeout(resolve, 5000));
                
                // إذا وصلنا هنا ولم يتم spawn، نكمل للإصدار التالي
                client.disconnect();
                
            } catch (error) {
                console.log(`⚠️ خطأ مع الإصدار ${version}: ${error.message}`);
                continue;
            }
        }
        ...
        if (ctx) {
            try {
                await ctx.editMessageText(`❌ فشل الاتصال مع كل الإصدارات.\n\n` +
                    `🔍 تأكد من:\n` +
                    `1. السيرفر شغال\n` +
                    `2. IP والبورت صحيح\n` +
                    `3. المكركة مفعلة\n` +
        ...
        await Servers.updateOne({ _id: server._id }, { $set: { status: 'خطأ' } });
    }
}
...
const addServerWizard = new Scenes.WizardScene(
    ...
    async (ctx) => {
        ...
        try {
            const sentMessage = await ctx.reply('الآن أرسل الـ IP أو رابط السيرفر.\n\n مثال :(play.example.com)', Markup.inlineKeyboard([
    ...
    async (ctx) => {
        ...
        if (isNaN(ctx.wizard.state.serverData.port)) {
            await ctx.reply('❌ البورت يجب أن يكون رقماً. يرجى إدخال رقم بورت صحيح.');
            return;
        }
...
if (duplicateOther) {
    ...
    return;
            ...
            try {
                await ctx.reply('❌ حدث خطأ أثناء إضافة السيرفر. يرجى المحاولة مرة أخرى.');
            } catch (e) { /* ignore */ }
            await ctx.scene.leave();
            return sendMainMenu(ctx);
        }
...
addChannelScene.enter((ctx) => ctx.reply('أرسل اسم القناة مع @ (مثال: @tor)\nللإلغاء أرسل /cancel 👇').catch(console.error));
...
userActionScene.on('text', async (ctx) => {
    ...
    if (targetId === ADMIN_ID) return ctx.reply('لا يمكن تطبيق هذا الإجراء على المطور الأساسي.');
    ...
    switch (action) {
        case 'ban':
            ...
            await ctx.reply(`✅ تم حظر المستخدم ${user.username || targetId}.`);
            break;

        case 'unban':
            await Users.updateOne({ userId: targetId }, { $set: { isBanned: false } });
            await ctx.reply(`✅ تم رفع الحظر عن ${user.username || targetId}.`);
            break;
...
// ✅ تحديث الاسم واليوزر فقط
bot.use(async (ctx, next) => {
    if (!ctx.from) return next();

    const userId = ctx.from.id;

// ✅ username مع @ أو "غير معروف"
...
if (!currentName) currentName = "غير معروف";
    ...
    if (user) {
        ...
        if (updateNeeded) {
            ...
            console.log(`✅ تم تحديث بيانات ${userId} (username/name) فقط`);
        }
    } else {
        // إذا أول مرة يدخل، نخزن كلشي + joinedAt مرة وحدة
        await Users.create({
         userId,
         username: ctx.from.username ? `@${ctx.from.username}` : "غير معروف",
         name: currentName || "غير معروف"
          // joinedAt و باقي الحقول يضيفها الـ create مثل ما عندك
  });
...
` +
                     `📌 *للمساعدة التقنية تفضل معنا في القناة:*
` +
                     `@IBR_Chaneel`;
    ...
    try {
        ...
        if (!user) {
            user = await Users.create({
                ...
                // Add fields to store IP and server name if applicable, initialized to null or empty
                // For example: lastKnownIp: null, lastServerName: null
            });
        }
...
async function showMyServers(ctx, message) {
    ...
    const servers = allServers
        ...
        .sort((a, b) => {
            // سحب الرقم من الاسم S-1, S-2 ...
            const numA = parseInt(a.serverName.replace('S-', ''), 10);
            const numB = parseInt(b.serverName.replace('S-', ''), 10);
            return numA - numB; // ترتيب تصاعدي
        });
...
bot.action(/delete_do:(.+)/, async (ctx) => { 
    try { await ctx.answerCbQuery('جاري الحذف...'); } catch(e) {/*ignore*/} 
    ...
    await showMyServers(ctx, '✅ تم حذف السيرفر.'); 
});
...
bot.action('admin_versions', async (ctx) => { 
    try { 
        await ctx.editMessageText(
            '⚙️ إدارة إصدارات ماينكرافت.',
            Markup.inlineKeyboard([
                [Markup.button.callback('📋 عرض الكل', 'admin_list_versions')],
                [Markup.button.callback('➕ إضافة', 'admin_add_version'), Markup.button.callback('➖ حذف', 'admin_delete_version')],
                [Markup.button.callback('🔙 رجوع', 'admin_panel')]
            ])
        ); 
    } catch(e) { /* ignore */ } 
...
bot.action('admin_list_versions', async (ctx) => {
    ...
    try {
        await ctx.editMessageText(bedrockText, { 
            reply_markup: { 
                inline_keyboard: [[Markup.button.callback('🔙 رجوع', 'admin_versions')]] 
            } 
        });
    } catch (e) { /* ignore */ }
});
...
async function checkServersStatus() {
    ...
    for (const [key, client] of activeClients.entries()) {
        try {
            ...
            // هنا نتأكد فعلياً من حالة الكلاينت
            if (client && (client.connected || client.isAlive)) {
                isOnline = true;
            } else {
            ...
            console.error(`⚠️ خطأ بفحص السيرفر ${key}:`, e);
            await Servers.updateOne(
                { _id: serverId, userId },
...
// ⏰ خليها تشتغل كل 5 ساعات
setInterval(checkServersStatus, 18000000);
